  ...
  return (
    ...
    >
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <PostAuthorRow
          author={post.author}
          humanHref={post.is_mock ? "#" : `/profile/${post.user_id}`}
          meta={formatLocalTime(post.created_at, t)}
          badge={
            followed ? (
              <span className="rounded-full border border-emerald-100/20 bg-emerald-100/[0.06] px-2 py-0.5 text-[10px] font-medium text-emerald-100/75">
                {t("follow.followedBadge")}
              </span>
            ) : null
          }
        />
